...
pub mod spelltroum_tournament {
    ...
    pub fn deposit<'info>(
        ...
        // Сразу забираем PDA‐bump из контекста
        let bump = *ctx.bumps.get("player_balance").unwrap();
        let player = &mut ctx.accounts.player_balance;

        require!(amount > 0, CustomError::InvalidAmount);

        // При первом init_if_needed amount будет 0, bump и owner — default
        // Здесь устанавливаем owner и bump
        player.owner = *ctx.accounts.owner.key;
        player.bump = bump;

        // А потом уже прибавляем
