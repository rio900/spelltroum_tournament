...
pub mod spelltroum_tournament {
    ...
    pub fn deposit<'info>(
        ...
        // Ссылка на аккаунт PDA
        let player = &mut ctx.accounts.player_balance;
        // Достаём bump, под которым PDA было создано
        let bump = *ctx.bumps.get("player_balance").expect("Bump not found");

        // Проверяем, что вносим не ноль
        require!(amount > 0, CustomError::InvalidAmount);

        // Инициализируем поля owner и bump
